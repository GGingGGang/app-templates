...
# 1) build 단위: java-app 루트에서 k8s-gitops/, 스크립트, README 제외하고 복사
(
  cd "$TPL_DIR"
...
# 2) gitops 단위: manifests/java-app -> manifests/<SVC>, apps/java-app.yaml -> <SVC>.yaml
cp -r "$TPL_DIR/k8s-gitops/." "$GITOPS"/
mv "$GITOPS/manifests/java-app"        "$GITOPS/manifests/$SVC"
mv "$GITOPS/argocd/apps/java-app.yaml" "$GITOPS/argocd/apps/$SVC.yaml"
...
다음 (명령만, 직접 실행):
  cd "$BUILD" && gradle -q bootJar   # 로컬 빌드 확인 (선택, gradle 필요 — CI 는 Dockerfile 안에서 빌드)
