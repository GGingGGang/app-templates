...
#
# ORG : GitHub owner (gitops repoURL 은 대소문자 유지, 이미지 경로는 소문자로 변환)
...
# 1) build 단위: node-app 루트에서 k8s-gitops/, 스크립트, README 제외하고 복사
(
  cd "$TPL_DIR"
...
# 2) gitops 단위: manifests/node-app -> manifests/<SVC>, apps/node-app.yaml -> <SVC>.yaml
cp -r "$TPL_DIR/k8s-gitops/." "$GITOPS"/
mv "$GITOPS/manifests/node-app"        "$GITOPS/manifests/$SVC"
mv "$GITOPS/argocd/apps/node-app.yaml" "$GITOPS/argocd/apps/$SVC.yaml"
...
손으로 확인 (sed 로 안 되는 배선):
  1) jenkins-shared-library services.yaml 의 services: 에 '$SVC': language: node 등록
